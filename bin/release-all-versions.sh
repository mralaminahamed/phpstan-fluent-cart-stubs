#!/usr/bin/env bash
#
# Generate Fluent Cart stubs for all available versions.
#
# This script:
# 1. Fetches all available Fluent Cart versions from WordPress.org
...
OUTPUT_FILE="$ROOT_DIR/fluent-cart_versions.txt"
PLUGIN_NAME="fluent-cart"
PLUGIN_API_URL="https://api.wordpress.org/plugins/info/1.0/fluent-cart.json"
...
while IFS= read -r VERSION; do
    ...
    git commit -m "Generate stubs for Fluent Cart ${VERSION}" || true
    git tag "v${VERSION}"
